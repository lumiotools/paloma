...
const languageRef = useRef<"hindi" | "english">("hindi")
    ...
    const handleDataChannelMessage = (event: MessageEvent) => {
      try {
        ...
        if (message.type === "text" && message.text) {
          ...
          setApiError(typeof message.error === "string" ? message.error : "Voice service error")
        }
      } catch (error) {
  ...
  const handleStopRecording = async () => {
    ...
    try {
      if (connectionRef.current) {
        await stopRealtimeSession(connectionRef.current, dataChannelRef, conversationHistoryRef)
        connectionRef.current = null
        dataChannelRef.current = null
      }
  ...
  {Array.from({ length: 11 }).map((_, i) => {
    ...
    if (isAiSpeaking || isUserSpeaking) {
      ...
      const now = Date.now();
      const seed = now / (isAiSpeaking ? 180 : 150) + (i * Math.PI) / 5.5;
      ...
      waveBarHeights.current[i] = Math.max(waveBarHeights.current[i] * 0.9, 3);
      height = waveBarHeights.current[i];
    }

    return (
      <div
        key={`wave-bar-${i}-${isAiSpeaking ? 'ai' : isUserSpeaking ? 'user' : 'idle'}`}
